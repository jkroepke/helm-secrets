...
@test "secret-backend: helm secrets -b" {
    ...
    run "${HELM_BIN}" secrets -b nonexists dec "${FILE}"
...
@test "secret-backend: helm secrets --backend" {
    ...
    run "${HELM_BIN}" secrets --backend nonexists dec "${FILE}"
    assert_output --partial "Can't find secret backend: nonexists"
    assert_failure
}
...
@test "secret-backend: helm secrets + env HELM_SECRETS_BACKEND" {
    ...
    run env HELM_SECRETS_BACKEND=nonexists WSLENV="HELM_SECRETS_BACKEND:${WSLENV}" "${HELM_BIN}" secrets dec "${FILE}"
    assert_output --partial "Can't find secret backend: nonexists"
    assert_failure
}
...
@test "secret-backend: helm secrets -b noop" {
    ...
    run "${HELM_BIN}" secrets -b noop dec "${FILE}"
...
@test "secret-backend: helm secrets --backend noop" {
    ...
    run "${HELM_BIN}" secrets --backend noop dec "${FILE}"
    assert_output --partial 'sops:'
    assert_success
}
...
@test "secret-backend: helm secrets -b noop + q flag" {
    ...
    run "${HELM_BIN}" secrets -q -b noop dec "${FILE}"
    assert_output --partial 'sops:'
    assert_success
}
...
@test "secret-backend: helm secrets + env HELM_SECRETS_BACKEND=noop" {
    ...
    run env HELM_SECRETS_BACKEND=noop WSLENV="HELM_SECRETS_BACKEND:${WSLENV}" "${HELM_BIN}" secrets dec "${FILE}"
    assert_output --partial 'sops:'
    assert_success
}
...
@test "secret-backend: helm secrets + prefer cli arg -b noop over env" {
    ...
    run env HELM_SECRETS_BACKEND=sops WSLENV="HELM_SECRETS_BACKEND:${WSLENV}" "${HELM_BIN}" secrets -b noop dec "${FILE}"
    assert_output --partial 'sops:'
    assert_success
}
...
@test "secret-backend: helm secrets --backend assets/custom-backend.sh" {
    ...
    run "${HELM_BIN}" secrets --backend "${TEST_TEMP_DIR}/assets/custom-backend.sh" dec "${FILE}"
...
@test "secret-backend: helm secrets + env HELM_SECRETS_BACKEND=assets/custom-backend.sh" {
    ...
    run env HELM_SECRETS_BACKEND="${TEST_TEMP_DIR}/assets/custom-backend.sh" WSLENV="HELM_SECRETS_BACKEND:${WSLENV}" "${HELM_BIN}" secrets dec "${FILE}"
    refute_output --partial '!vault'
    assert_output --partial 'production#global_secret'
    assert_success
}
